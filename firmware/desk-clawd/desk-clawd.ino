/*
 * ╔══════════════════════════════════════════════════════════════╗
 *   DESK CLAWD — ESP32-C3 Super Mini + ST7789 1.54" 240×240
 *                + Traffic Light (Claude Code status indicator)
 *
 *   Display wiring:
 ...
 *     Yellow → GPIO 9
 *     Green  → GPIO 5
 *     GND    → GND (daisy-chain with display)
 *
 *   Light states via HTTP (curl):
 ...
 *
 *   WiFi AP: "Desk-Clawd"  pw: clawd1234  → http://192.168.4.1
 *   WiFi STA: edit secrets.h for your home WiFi SSID/password
 *   Serial: USB CDC  → 115200 baud (traffic light commands)
 * ╚══════════════════════════════════════════════════════════════╝
 */
...
LightState greenLed  = {LED_GREEN,  true,  false, 0,   0, true};
LightState yellowLed = {LED_YELLOW, false, true,  250, 0, false};
LightState redLed    = {LED_RED,    false, true,  250, 0, false};
...
void setLedMode(LightState &light, bool active, bool blink, unsigned long interval) {
  light.active = active;
  light.blink = blink;
...
void updateLed(LightState &light) {
  if (!light.active) {
...
void setTrafficLight(const char* raw) {
  // ── Named states ───────────────────────────────────────────
  if (strcmp(raw, "idle") == 0 || strcmp(raw, "green") == 0) {
    greenLed.active = false; yellowLed.active = false; redLed.active = false;
    setLedMode(greenLed, true, false, 0);
    return;
  }
  if (strcmp(raw, "thinking") == 0 || strcmp(raw, "yellow") == 0) {
    greenLed.active = false; yellowLed.active = false; redLed.active = false;
    setLedMode(yellowLed, true, true, 250);
    return;
  }
  if (strcmp(raw, "running") == 0 || strcmp(raw, "red") == 0) {
    greenLed.active = false; yellowLed.active = false; redLed.active = false;
    setLedMode(redLed, true, true, 250);
    return;
  }
  if (strcmp(raw, "off") == 0) {
    greenLed.active = false; yellowLed.active = false; redLed.active = false;
  ...
  // ── Direct command: {G|Y|R}:{on|off|blink}[:{ms}] ─────────
  char light = raw[0];
  if (light != 'G' && light != 'Y' && light != 'R') return;

  LightState* target = NULL;
  if (light == 'G') target = &greenLed;
  else if (light == 'Y') target = &yellowLed;
  else target = &redLed;

  int firstColon = -1;
  for (int i = 0; raw[i]; i++) {
    if (raw[i] == ':') { firstColon = i; break; }
  }
  if (firstColon < 0) {
    // bare "G" / "Y" / "R" → default behaviour like named states
    greenLed.active = false; yellowLed.active = false; redLed.active = false;
    if (light == 'G') setLedMode(greenLed, true, false, 0);
    else setLedMode(*target, true, true, 250);
    return;
  }

  int secondColon = -1;
  for (int i = firstColon + 1; raw[i]; i++) {
    if (raw[i] == ':') { secondColon = i; break; }
  }

  char mode[8] = {0};
  int modeEnd = (secondColon > 0 ? secondColon : strlen(raw));
  int modeLen = modeEnd - firstColon - 1;
  if (modeLen > 7) modeLen = 7;
  strncpy(mode, raw + firstColon + 1, modeLen);

  unsigned long interval = 250;
  if (secondColon > 0) {
    interval = atoi(raw + secondColon + 1);
  ...
  if (strcmp(mode, "on") == 0) {
    setLedMode(*target, true, false, interval);
  } else if (strcmp(mode, "blink") == 0) {
    setLedMode(*target, true, true, interval);
  } else if (strcmp(mode, "off") == 0) {
    setLedMode(*target, false, false, interval);
  }
}
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\n' || c == '\r') {
      if (serialBuffer.length() > 0) {
        serialBuffer.trim();
        setTrafficLight(serialBuffer.c_str());
        serialBuffer = "";
      }
    } else {
      serialBuffer += c;
      if (serialBuffer.length() > 63) serialBuffer = "";
    }
  }
}
...
void termDrawHeader() {
  ...
  tft.setCursor(TERM_PAD_X, 4); tft.print("clawd@desk terminal");
...
<div class="hdr">
  ...
  <div class=<div class="sitename">DESK CLAWD "sitename">DESK CLAWD CLAWD &middot; MOCHI &middot; CONTROLLERmiddot; CONTROLLER</div>middot; CONTROLLER</div>
</div>
...
void routeLight() {
  if (!server.hasArg("state") || server.arg("state").isEmpty()) {
    server.send(400, "application/json", "{\"e\":1}"); return;
  }
  setTrafficLight(server.arg("state").c_str());
  server.send(200, "application/json", "{\"ok\":1}");
}
...
void setup() {
  ...
  setLedMode(greenLed, true, false, 0);  // start with green
  ...
  tft.setCursor(DISP_W / 2 - 54, DISP_H / 2 - 22); tft.print("Desk");
  tft.setCursor(DISP_W / 2 - 54, DISP_H / 2 + 14); tft.print("Clawd");
  delay(1200);
  ...
  // Try home WiFi for up to 6 seconds
  ...
  tft.fillRect(0, 0, DISP_W, 4, C_ORANGE);
  tft.setTextColor(C_WHITE);  tft.setTextSize(2);
  tft.setCursor(12, 16);  tft.print("WiFi: Desk-Clawd");
  ...
  if (homeIP.length() > 0) {
    tft.setTextColor(C_GREEN); tft.setTextSize(1);
    tft.setCursor(12, 124); tft.print("IP: " + homeIP);
  } else {
    tft.setTextColor(C_MUTED); tft.setTextSize(1);
    tft.setCursor(12, 124); tft.print("home WiFi not found");
  }
  tft.setTextColor(C_GREEN); tft.setTextSize(1);
  tft.setCursor(12, 144); tft.print("press any button to start");
  ...
  server.on("/light",       HTTP_GET, routeLight);
